...
- **Validate all external input at system boundaries.** API endpoints, message handlers, and file uploads are trust boundaries. Use FluentValidation or built-in validation attributes before data reaches domain logic.
...
// EF Core interpolation is parameterized - above is safe
...
- **Always add `[Authorize]` or `[AllowAnonymous]` explicitly on every controller or endpoint.** Ambiguous auth is a security hole. Never rely on a global default without also making intent explicit at endpoint level.
...
// DON'T - unmarked endpoint inherits whatever global default is
